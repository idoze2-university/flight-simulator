...
bool isBadCharacter(char c, int status) {
    switch (status) {
        case 0:
            ...
            break;
        case 1:
            if (!(isLowerChar(c)) && !(isUpperChar(c)) && !(isDigit(c)) && c != '_') { return true; }
            break;
        case 2:
            if (!(isLowerChar(c)) && !(isUpperChar(c)) && !(isDigit(c)) && c != '_') {
                if (!isMathToken(c)) { return true; }
            }
            break;
        default:
            return false;
...
void var_bad_line(std::string line) {
    ...
    if (line.find("<-") != string::npos) {
        ...
        if (argument.find("sim") != 0) { throw e; }
        if (argument.find("(") != 3) { throw e; }
        if (argument[argument.size() - 1] != ')') { throw e; }
    } else if (line.find("->") != string::npos) {
        badOperator_flag = false;
        char c;
        unsigned int i = 3, pos = line.find("->");
